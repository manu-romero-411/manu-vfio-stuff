...
args=(
	-enable-kvm -m "$RAM" -cpu "$CPU",kvm=on,vendor=GenuineIntel,+invtsc,vmware-cpuid-freq=on,+pcid,+ssse3,+sse4.2,+popcnt,+avx,+aes,+xsave,+xsaveopt,check
	...
	-device usb-tablet
	-usb
#	-device usb-host,hostbus=1,hostaddr=27
#	-device usb-host,hostbus=1,hostaddr=28
